        ...
        if request.method == 'POST':
            ...
            if listing_form.is_valid() and location_form.is_valid():
              ...
              return redirect('home') 
                ...
                messages.error(
                    request, f'An error occured while trying to edit listing.')
                return reload()
        else:
...
"""@login_required
def inquire_lisitng_using_email(request,id):
    listing = get_object_or_404(Listing, id=id)
    try: 
        emailSubject = f'{request.user.username} is interested in {listing.model}'
        emailMessage = f'Hi {listing.seller.user.username}, {request.username} is interested in your {listing.model} listing on AutoMax'
